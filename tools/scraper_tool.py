...
class ScrapeWebsiteTool(BaseTool):
	...
	def _run(
		...
		**kwargs: Any,
		...
		for element in parsed(["script", "style", "header", "footer", "nav", "aside"]):
			element.decompose()

		main_content = parsed.find('main')
		if not main_content:
			main_content = parsed.find('article')
		if not main_content:
			main_content = parsed.find('div', id='content')
        
		if main_content:
			text = main_content.get_text()
		else:
			text = parsed.body.get_text() if parsed.body else parsed.get_text()
